...
(function(){
     ...
     */
    var dims;
//Wizzud [scroll]: within the private variables section, after the line ... var dims; ..., insert 13...
var scrollBar
  , getScrollbarDim = function(){
      var ta = client.isSafari3
        , s = 'position:absolute;top:-200px;left:-200px;width:50px;overflow:hidden;'
        , p = (ta ? 'scroll' : 'offset') + 'Width'
        , t = ta ? '<textarea id="shadowbox_GSD" cols="10" rows="2" style="'+s+'"></textarea>'
                 : '<div id="shadowbox_GSD" style="'+s+'height:50px;"><div style="height:100px;"></div></div>';
      SL.append(document.body, t);
      var x = SL.get('shadowbox_GSD'), y = ta ? x : x.firstChild, w = y[p];
      x.style.overflow = 'scroll';
      scrollBar = w - y[p];
      document.body.removeChild(x);
 };
//Wizzud [scroll]: end of insert
    /**
            ...
            var fn = function(){
                resizeContent(function(){
                    ...
                    showBars(function(){
                        ...
                        //Wizzud [scroll]: in loadContent(), before the line ... toggleLoading(false, function(){ ..., insert 1...
SL.setStyle(SL.get('shadowbox_body_inner'), {overflow:dims.scroll?'auto':'hidden'});
//Wizzud [scroll]: end of insert
...
//Wizzud [scroll]: in setDimensions(), after the line ... var view_w = SL.getViewportWidth(); ..., insert 1...
        var scrlX, scrlY, scrlXY;
//Wizzud [scroll]: end of insert
        ...
        if(w + extra_w >= view_w){
            w = view_w - extra_w;
        }else{ scrlY = (view_w - extra_w - w) >= scrollBar; } 
        ...
        if(h + extra_h >= view_h){
            h = view_h - extra_h;
        }else{ scrlX = (view_h - extra_h - h) >= scrollBar; } 
        ...
        var handle = options.handleOversize;
         if(resizable && {resize:1, drag:1, scroll:1}[handle]){ 
  ...
  scrlXY = (height > h || width > w);
  if(width <= w && scrlXY && scrlY){ w += scrollBar; }else if(height <= h && scrlXY && scrlX){ h += scrollBar; }
//Wizzud [scroll]: end of insert
            }else{
